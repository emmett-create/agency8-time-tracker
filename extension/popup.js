// Agency 8 — Time Tracker popup.js         
  ...
  function startEditEntry(entry) {                                                                                                                                                                          
    ...
    entryEl.innerHTML = `
      <div class="edit-form">
        ...
        <div class="field-row">
          <div class="field">
            <label>Date</label>
            <input type="date" class="edit-date" value="${entry.entry_date}">
          </div>
          <div class="field">                 
        ...
        <div class="edit-actions">                                                                                                                                                                          
          ...
          <button class="btn-split-edit">Split</button>
    ...
    entryEl.querySelector('.btn-split-edit').addEventListener('click', () => startSplitEntry(entry, entryEl));
    entryEl.querySelector('.btn-cancel-edit').addEventListener('click', () => loadEntriesForDate(selectedDate));
  }

  function startSplitEntry(entry, entryEl) {
    const total = entry.duration_minutes;
    ...
    entryEl.innerHTML = `
      <div class="split-form">
        <div class="split-header">Split ${formatMinutes(total)}</div>
        <div class="split-rows">
          ${buildRowHtml(0)}
          ${buildRowHtml(1)}
        </div>
        <button class="btn-add-split">+ Add row</button>
        <div class="split-status">
          <span class="split-remaining">${total} min remaining</span>
        </div>
        <div class="edit-actions">
          <button class="btn-save-split" disabled>Save Split</button>
          <button class="btn-cancel-split">Cancel</button>
        </div>
      </div>
    `;

    function updateStatus() {
      const inputs = entryEl.querySelectorAll('.split-min');
      const used = [...inputs].reduce((s, i) => s + (parseInt(i.value) || 0), 0);
      const rem = total - used;
      const label = entryEl.querySelector('.split-remaining');
      const saveBtn = entryEl.querySelector('.btn-save-split');
      if (rem === 0) {
        label.textContent = '✓ Ready to save';
        label.style.color = '#43e97b';
        saveBtn.disabled = false;
      } else if (rem > 0) {
        label.textContent = `${rem} min remaining`;
        label.style.color = '#666';
        saveBtn.disabled = true;
      } else {
        label.textContent = `${Math.abs(rem)} min over`;
    ...
    entryEl.addEventListener('input', e => {
      if (e.target.classList.contains('split-min')) updateStatus();
    });

    entryEl.querySelector('.btn-add-split').addEventListener('click', () => {
      const rowsEl = entryEl.querySelector('.split-rows');
      const index = rowsEl.querySelectorAll('.split-row').length;
      rowsEl.insertAdjacentHTML('beforeend', buildRowHtml(index));
      rowsEl.lastElementChild.querySelector('.btn-remove-row').addEventListener('click', function() {
    ...
    entryEl.querySelectorAll('.btn-remove-row').forEach(btn => {
    ...
    entryEl.querySelector('.btn-save-split').addEventListener('click', async () => {
      ...
      await fetch(`${settings.supabaseUrl}/rest/v1/time_entries?id=eq.${entry.id}`, {
        method: 'DELETE',
        headers: { 'apikey': settings.supabaseKey, 'Authorization': `Bearer ${settings.supabaseKey}` },
      });

      for (const split of splits) {
        await fetch(`${settings.supabaseUrl}/rest/v1/time_entries`, {
          method: 'POST',
          headers: {
            'Content-Type':  'application/json',
            'apikey':        settings.supabaseKey,
    ...
    entryEl.querySelector('.btn-cancel-split').addEventListener('click', () => loadEntriesForDate(selectedDate));
  }                                       
